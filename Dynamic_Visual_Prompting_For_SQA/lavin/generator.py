...
class LaVIN_Generator:
    ...
    def generate(
        ...
        image_embeds, image_feats = self.model.backbone.encode_image(images)
        image_embeds = self.model.adapter_proj(image_embeds)


        prompt_len = []
        for prompt in prompts:
            prompt_len.append(len(self.tokenizer.encode(prompt, bos=False, eos=False)))
        ...
        for i in range(len(token_embeds)):
            if indicators[i]==1:
                ...
                image_token_embed=torch.cat([token_embeds[i,:pos],image_embeds[i],self.model.image_params,token_embeds[i,pos+n_feats:]],0)
                token_embeds[i]=image_token_embed

        prefix_img_len = len(prefix_img_token)
        prefix_nonimg_len = len(non_prefix_img_token)

        image_prompt_index = 1 + prefix_img_len + 7
        text_prompt_index = 1 + prefix_nonimg_len
