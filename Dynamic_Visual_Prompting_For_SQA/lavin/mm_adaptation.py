...
def LaVIN(args):
    ...
    model_args: ModelArgs = ModelArgs(
        max_seq_len=args.max_seq_len, max_batch_size=32,hidden_proj=args.hidden_proj,drop_path=args.drop_path, insert_layer = args.insert_layer, stage = args.kab_app, **params
    )
    ...
    for name, param in llama.named_parameters():
        if 'adapter' in name or 'cross_attn' in name or 'image_params' in name:
